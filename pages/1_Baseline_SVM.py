...
st.set_page_config(page_title="Baseline SVM", page_icon="🤖", layout="centered")

@st.cache_resource
def load_models():
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    models_dir = os.path.join(base, 'models')
    with open(os.path.join(models_dir, 'baseline_svm.pkl'), 'rb') as f:
        model = pickle.load(f)
...
st.title("🤖 Baseline SVM Prediction")
st.markdown("Predicts using all **5,000 TF-IDF features** with no feature selection.")
st.markdown("**Model Accuracy: 95.40% | F1 Score: 95.55%**")
st.divider()
...
if st.button("🔍 Predict", use_container_width=True):
    if not article.strip():
        ...
        st.markdown(f"- **Features used:** 5,000")
        st.markdown(f"- **Model:** Linear SVM (Baseline)")